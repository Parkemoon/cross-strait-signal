...
- `view` state (`"feed"` | `"review"` | `"economy"` | `"trade"`) controls the center column on desktop. When `view === "review"` the center column renders `<ReviewQueue />`; `"economy"` → `<EconomyTab />`; `"trade"` → `<TradeAccessTab />`.
- The 3-column grid collapses to 2 columns (hides the Social Pulse right aside) when `view` is `"economy"` or `"trade"`, so wide tables/charts get full width.
...
| A new metric for `cross_strait_population` | Update the seed script or scraper to produce the metric; the `PeopleSection` JSX pulls a small fixed set of metrics (latest residence permit, census, spouse stock, 台胞证 holders) so add a KPI card and/or chart for new metrics that should be surfaced |
