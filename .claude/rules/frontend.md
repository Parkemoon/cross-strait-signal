...
- `view` state (`"feed"` | `"review"` | `"economy"` | `"trade"` | `"people"` | `"military"`) controls the center column on desktop. `"review"` → `<ReviewQueue />`; `"economy"` → `<EconomyTab />`; `"trade"` → `<TradeAccessTab />`; `"people"` → `<PeopleTab />`; `"military"` → `<MilitaryTab />`.
- The 3-column grid collapses to 2 columns (hides the Social Pulse right aside) when `view` is `"economy"`, `"trade"`, `"people"`, or `"military"`, so wide tables/charts/maps get full width.
