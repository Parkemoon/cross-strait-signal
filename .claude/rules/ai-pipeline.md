...
- **Aggregate-vs-intensity rule** — when the article reports a top-line aggregate followed by an intensity breakdown in parentheses ("有45.7%滿意（其中15.9%很滿意、29.8%還算滿意）"), the prompt instructs the model to extract ONLY the aggregate (3 options: 滿意 / 不滿意 / 未明確) NOT the 5-option intensity breakdown. The aggregate is what's comparable across waves; the intensity breakdown breaks trend continuity. Rule scope is limited to binary-with-intensity scales (satisfied/dissatisfied, trust/distrust, favourable/unfavourable, good/bad). Genuine multi-point Likert scales (agree/disagree five-point) and multi-option position scales (統獨 7-step) keep all their options. The 2026-05 My-Formosa wave was the canonical incident: 10 questions stored disaggregated, breaking trend charts under their existing question_keys; fixed by reaggregating + this prompt rule.
