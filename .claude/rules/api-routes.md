...
Filter params: `topic`, `sentiment`, `source_place`, `source_name`, `bias`, `urgency`, `escalation_only`, `entity`, `search`, `include_pending`, `alt_model` + `alt_arm`.
...
Scoping filters are centralised in `_build_filter_clause(topic, source_place, urgency, escalation_only, entity, source_name, bias)` — add new article-level filters there, not inline. When active, sentiment aggregations scope to those filters while topics/sources/entities/escalation signals stay global. The response always includes `global_avg_sentiment_score` and `global_sentiment_by_place` for ghost-dot comparison in the sidebar.

`alt_model` + `alt_arm` (both together, admin-gated via `Depends(is_admin)` — same convention as `articles.py`) apply the **alt-model lens** to the aggregates: an INNER JOIN on `alt_model_analysis` (exact model/arm, `outcome='ok'`) narrows every aggregation to the swept subset, and sentiment/topic columns read from the sweep (`alx` alias) instead of `ai`. The response echoes `alt_lens: {model, arm} | null` — the sidebar banner renders from this echo, not from client state. Under the lens it also carries `alt_baseline` (`{avg_sentiment_score, sentiment_by_place, sentiment_trend}`): PRODUCTION Gemini aggregates over the SAME swept subset, deliberately unfiltered — the sidebar's "Both" view overlays these as ghost dots + a dashed trend line, and same-subset scoping is what makes the comparison apples-to-apples. Anonymous callers passing the params get production stats. Three deliberate exceptions under the lens: `_build_filter_clause` scoping still evaluates against PRODUCTION `ai` values (matching the articles.py lens); `escalation_signals` stays production (it feeds the FlashTraffic editorial section, not an aggregate); `top_entities` counts production entity extractions merely narrowed to swept articles (sweeps don't write the `entities` table). When adding a new aggregation to this route, it must take `{ALT_JOIN}` and read analysis columns via the `{X}` prefix, with `(*alt_params, ...)` prepended to its bind params. `sentiment_by_place` normalises raw `s.place` values into four display buckets (PRC/TW/HK/INTL) via a `PLACE_BUCKET` SQL CASE expression — never group by raw `s.place` in this query or you'll get duplicate rows for UK, SG, etc.
...
- `GET /api/visits/{id}/coverage` — every article reporting the visit: the visit family's own articles (the keeper plus rows merged into it — a recursive CTE over `merged_into_id`, so merge chains count); one entry per article, oldest first. Feed event-cluster siblings are deliberately NOT included (built in on 2026-09-08, dropped 2026-09-13: title-Jaccard clustering leaked neighbour stories such as a Han Kuo-yu budget piece under a Cheng Li-wun trip) — an outlet reaches the card only through a visit row the analyst approved or merged, so the merge picker is the lever for adding coverage. Each carries `side` (`TW`/`PRC`/null via `shared/source_side.py` — the LinkedIn selector's rule) and `outlet` (section feeds collapsed to the publication), plus title/summary overrides and sentiment. `counts` = `{articles, outlets, TW, PRC, other}`. `Depends(is_admin)`: public callers get feed-visible articles only (`_VISIBLE_ARTICLE`) and 404 on a non-approved visit; admins see `is_hidden=0` articles whatever their approval state. `/list` attaches the same `counts` per visit as `coverage` (one batched query for the page) so the card can show the toggle line without a fetch.
...
- `PATCH /api/positions/text` (**admin** — `Depends(require_admin)`) — inline content editing for the co-writing flow: body `{"path": ["actors", 0, "intro_en"], "value": "…"}` replaces ONE string field, addressed by its JSON path from the file root, and atomically rewrites the file (json.dumps-normalised — the file was normalised to that format when this landed, so edit diffs stay minimal). Deliberately **strings only**: structural changes (new entries, sources, ids) stay hand-edited so `tests/test_positions_content.py`'s shape invariants can't be broken from the UI. Guards (validated in `_apply_text_edit`, unit-tested in `tests/test_positions_edit.py`): path root must be `actors`/`concepts`/`_meta`; `_comment*` keys and the structural leaves `id`/`actor_id`/`party`/`depth`/`iso` are rejected (keep that set in lockstep with `EDIT_EXCLUDE` in `PositionsTab.jsx`); the leaf must exist and already be a string. Edits land as an uncommitted git diff on the worktree's JSON — review + commit is still a session task. NOTE for deploys: an edit made on the PROD worktree dirties a tracked file — commit or stash it before `server_deploy.sh` pulls.
...
`zones` (add `?geometry=1` for polygons), `summary?days=`, `daily?zone=|group=&force=&days=`, `monthly`, `vessels?force=&status=&anomalies=1`, `vessels/{mmsi}`, `encounters?days=` (v1 "encounter" = a CCG hull and a CGA hull in the SAME zone on the SAME day — co-presence, not interaction; label it so). Every aggregate applies `_NOT_REJECTED` (roster status) and counts distinct hulls by `_HULL_KEY` (`force:hull_no` → `force:name` → mmsi, via `_VESSEL_JOIN`) because ~10 CCG hulls broadcast under two MMSIs. `summary` carries `caveats[]` ({key, scope, en} — text from `data/site_copy.json`, scope from `CAVEAT_SCOPES`), `coverage_start`, `anomalies` (hulls with AIS-identity flags — a fact count, NOT a queue) and `unreviewed` (status=`auto` hulls with presence in the window — the real review residual after `triage_roster`; the UI badges this one); the frontend MUST render the scoped caveat next to any chart of that scope (the series is an AIS-visible floor). **Display is two-force** (`DISPLAY_FORCES` = CCG/CGA, Ed's call 2026-08-31): JCG/USCG are still pulled and roster-classified but excluded from every public aggregate (JCG = Senkaku/Yonaguni overspill in the east box, USCG = 2 hull-days ever); an explicit `?force=JCG` on `/daily` / `/monthly` still returns the hidden series. Admin `PATCH vessels/{mmsi}` (status/force/hull_no/notes) — changing `force` rewrites the hull's presence rows. `group` = zone area (kinmen/matsu/median/contiguous/pratas/east/taiwan_bank); `kind` = prohibited/restricted/band/sector/radius/box/shoal.

## `/api/military/maritime/*` (`api/routes/maritime.py`, Phase 2g)

The civilian-fleet + radar layer under the coast-guard section; same framing rules (AIS presence = FLOOR; SAR unmatched detections = radar-visible CEILING, counted per satellite pass NOT hull-days; GFW class is coarse so OTHER is "non-fishing", never "dredger"; militia = the cited study's inference from ownership/subsidy/training records — never ours, never proof; the copy says so). `summary?days=` → per (flag, class) hull-days this window vs previous (`civil[]`, CHN/TWN only), per-zone CHN fishing / non-fishing / cargo + SAR matched/unmatched (`zones[]`, joined to the zone geojson so every zone appears), `sar` headline (current/previous window on the SAR series' OWN latest date — it lags the AIS series by ~2 months; `dark_share` = unmatched ÷ all), `militia` ({roster_size, seen[] = roster hulls with presence in the trailing year, since}), `caveats[]` from `maritime.caveat.<key>` + `CAVEAT_SCOPES` (scopes `all` / `sar` / `militia` / `taiwan_bank`), `hull_zones`, `class_labels`, `coverage_start`, `sar_latest_date`. `civil/monthly?flag=CHN|TWN&zone=|group=&months=` and `civil/daily` (hull-days by class), `civil/vessels?zone=|group=&vessel_class=&militia=1&days=&limit=` (named hulls = the `keep_hull` subset only), `sar/monthly` / `sar/daily` (matched / unmatched / CHN / TWN matched / pass_days / dark_share). All public reads; nothing here is analyst-gated.

- `GET /api/military/coast-guard/enforcement?region=TW|金門縣…&category=fishing_prc&months=` — the mirror series (monthly national — monthly reports win, yearbook 表8-1 months fill the back-history to 2021-01; annual incl. manual fines/confiscations, county snapshots) plus `sources[]` ({source, source_ref, source_url, latest_period} — the distinct report PDFs the rows were parsed from) and `cga_stats_home`, so every 表8-1/表8-3 citation in the UI links to its document. `summary` also carries `enforcement` (trailing-12-reported-months PRC expelled/detained) so the KPI strip shows both directions.
