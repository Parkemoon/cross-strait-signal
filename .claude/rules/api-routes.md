...
- **Richness guard on approve:** refuses the approve (409) if any pending peer on the same `(pollster_id, fielded_start)` key carries MORE extracted questions than the row being approved. Without this, approving a 1-question news rehash before the full release page would silently NULL the richer peer's `pending_results_json` during the canonical-merge step — happened to the 2026-05 My-Formosa wave. Error message names the richer peer(s); the analyst clears it by approving the richest one first (the others get folded in automatically). The guard compares AI-extracted question counts in `pending_results_json`, not analyst `skip` decisions.
