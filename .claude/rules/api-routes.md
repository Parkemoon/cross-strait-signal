...
- `GET /api/visits/{id}/coverage` — every article reporting the visit: the visit family's own articles (the keeper plus rows merged into it — a recursive CTE over `merged_into_id`, so merge chains count) tagged `via='visit'`, plus every article sharing an `event_cluster_id` with one of them tagged `via='cluster'`; one entry per article, `visit` wins over `cluster`, oldest first. Each carries `side` (`TW`/`PRC`/null via `shared/source_side.py` — the LinkedIn selector's rule) and `outlet` (section feeds collapsed to the publication), plus title/summary overrides, sentiment and `cluster_id` cast to TEXT. `counts` = `{articles, outlets, TW, PRC, other}`. `Depends(is_admin)`: public callers get feed-visible articles only (`_VISIBLE_ARTICLE`) and 404 on a non-approved visit; admins see `is_hidden=0` siblings whatever their approval state. `/list` attaches the same `counts` per visit as `coverage` (one batched query for the page) so the card can show the toggle line without a fetch.
