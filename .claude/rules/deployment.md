...
Several sources use a self-hosted RSSHub instance on the server (`http://localhost:1200`) — People's Daily, Global Times, The Paper, Zaobao and RTHK Greater China. (The CT sections left RSSHub on 2026-09-24 for `chinatimes_scraper.py`; see China Times below.) It runs as a Docker container:

```bash
docker run -d --name rsshub --restart always -p 1200:1200 diygod/rsshub:chromium-bundled
```

The `chromium-bundled` tag was required for the CT routes, which used Puppeteer. Keep it until every remaining route has been checked without it. If these feeds return 0 entries, check `docker ps` to confirm the container is running. rsshub.app (the public instance) blocks automated clients — always use localhost.

**The chinatimes route patch is obsolete since 2026-09-24.** No source uses the route now, so a re-pull that loses the patch no longer matters. The history is kept below. **The chinatimes route was locally patched inside the container** (2026-07-21). Upstream RSSHub (still broken on master) has two bugs: (a) `const { category = 'realtimenews' } = ctx.req.param('category')` object-destructures a string, so the category is always ignored and every section serves the generic 即時新聞 feed — this silently fed the realtime firehose into CT Cross-Strait for 3 months and starved the other three CT sections to zero via dedup; (b) category pages use absolute hrefs which the route double-prefixes with the base URL (503). Both are patched in the container's built file `/app/dist/chinatimes-CoSBu9wp.mjs` (original at `.bak` alongside). **The patch survives container restarts but is LOST if the image is re-pulled or the container recreated** — after any `docker pull`/`docker run`, re-check `curl localhost:1200/chinatimes/politic` returns a 政治-titled feed, not 即時新聞, and re-apply if not (see SESSION_LOG 2026-07-21 for the patch strings).
...
## China Times scraper (2026-09-24)

`chinatimes_scraper.py` drives headful Chromium, so the server needs **`apt install xvfb`** (installed on the shared server 2026-09-23) on top of the Playwright system deps listed under CIFER in `scrapers.md`. The scraper starts its own Xvfb, so the pipeline cron line is unchanged. Deploying the switch: `server_deploy.sh`, then `venv/bin/python scripts/seed_sources.py` on the prod worktree (the four CT rows move from `rss` + localhost:1200 URLs to `html_scrape` + their chinatimes.com section pages). Then backfill the outage gap, **detached**: `setsid nohup venv/bin/python -u -m scraper.scrapers.chinatimes_scraper --max-pages 12 --all-pages >> /var/log/chinatimes-backfill.log 2>&1 < /dev/null &`. CT's 總覽 listing holds only 200 items (10 pages), so that is as far back as it reaches; a longer gap cannot be closed this way. The first run (2026-09-23, before the memory fix) saved 538 articles and had to be stopped when available memory fell to 234 MB. Watch `free -m` during any long run. The backfilled rows land unanalysed, and the next pipeline ticks put them through Tier 1 in the usual batch.
...
## Maritime civilian-fleet + SAR layer — deploy-time data steps (2026-09-06)

Migration 0012 ships `maritime_civil_*` / `maritime_sar_daily` / `maritime_pulls` EMPTY, and `check_scraper_health.py` carries `maritime:civil_presence` (12 d) + `maritime:sar` (100 d — GFW publishes SAR ~2 months late) checks that will alert stale until the tables fill. After the first deploy, from the prod worktree, **detached**: `scripts/backfill_maritime.py --start 2020-01-01 --kinds civil` (re-requests the same presence reports as the coast-guard backfill, ~880 pulls / ~10 h; `coast_guard_presence` untouched) and `scripts/backfill_maritime.py --start 2017-01-01 --end <two months ago> --kinds sar` (~1,100 pulls). GFW serialises 4Wings reports per token, so the 6-hourly tick sees 429s (client retries) while either runs — run them one after the other, not concurrently. Pipeline Steps 2n (civil rows ride on the presence pull) and 2p (SAR, daily 120-day re-pull) keep both current afterwards. The militia roster is a committed file (`data/maritime_militia_roster.json`); the API caches it at import, so a roster edit needs a service restart.
