...
- **Band C is the bridge finding**: the same weights that refuse Tiananmen-direct process sovereignty-dense article content through the analytical scaffold **without a single refusal** — consistent with the full V4F article corpus (13,715 ok / 0 refused / 0 errors as of 2026-08-16). Refusal here is a property of the *prompt shape*, not the topic.
...
1. **Refusal is prompt-shape-dependent.** The same weights: 40% refusal on bare Band-A questions, zero across 20 Band-C scaffold calls and 13,715 production-shaped article tasks (V4F, as of 2026-08-16). The analytical scaffold doesn't *evade* the sensitivity — it changes the regime so it never fires.
