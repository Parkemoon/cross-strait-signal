...
from shared.source_side import outlet_of, side_of
...
# ── Feed coverage ────────────────────────────────────────────────────────
#
# A visit row is extracted from ONE article, and the dedup / merge picker
# folds the other articles on the same trip into it as `merged` rows.
# Coverage = the visit family's own articles (keeper + merged rows, chains
# included), so both sides' reporting of one trip sits together on the
# card. Feed event-cluster siblings are deliberately NOT included: same-story
# clustering is title-Jaccard and leaked neighbour stories (a Han Kuo-yu
# budget piece under a Cheng Li-wun trip) — Ed dropped them 2026-09-13. An
# outlet reaches the card only through a visit row the analyst approved or
# merged. Sides come from shared/source_side.py (the LinkedIn selector's
# rule); an international outlet counts towards the article total but
# towards neither side.

_COVERAGE_SQL = """
WITH RECURSIVE fam(visit_id, id) AS (
    SELECT id, id FROM cross_strait_visits WHERE id IN ({ph})
    UNION ALL
    SELECT fam.visit_id, c.id FROM cross_strait_visits c
    JOIN fam ON c.merged_into_id = fam.id
    WHERE c.approval_status = 'merged'
),
own AS (
    SELECT DISTINCT fam.visit_id, v.article_id FROM fam
    JOIN cross_strait_visits v ON v.id = fam.id
)
SELECT r.visit_id,
       a.id, a.url, a.published_at,
       COALESCE(a.title_en_override, a.title_en) AS title_en, a.title_original,
       COALESCE(a.summary_en_override, ai.summary_en) AS summary_en,
       ai.topic_primary, ai.sentiment, ai.sentiment_score,
...
def _coverage_rows(conn, visit_ids, admin: bool = False) -> dict:
    """{visit_id: [article dict, …]} — one entry per distinct article,
    oldest first. Public callers see only feed-visible articles."""
    ids = [int(i) for i in visit_ids]
    if not ids:
        return {}
    visible = "a.is_hidden = 0" if admin else _VISIBLE_ARTICLE
    sql = _COVERAGE_SQL.format(ph=",".join("?" * len(ids)), visible=visible)
    out: dict = {i: {} for i in ids}
    for r in conn.execute(sql, ids):
        d = dict(r)
        vid = d.pop("visit_id")
        d["side"] = side_of(d["place"], d["bias"])
        d["outlet"] = outlet_of(d["source_name"])
        out[vid][d["id"]] = d
    return {vid: list(arts.values()) for vid, arts in out.items()}


def _coverage_counts(arts: list) -> dict:
    sides = {"TW": 0, "PRC": 0, "other": 0}
    for a in arts:
        sides[a["side"] or "other"] += 1
    return {"articles": len(arts), "outlets": len({a["outlet"] for a in arts}), **sides}
...
def list_visits(
    ...
    with db_conn() as conn:
        ...
        visits = [_shape(r) for r in rows]
        cov = _coverage_rows(conn, [v["id"] for v in visits])
    for v in visits:
        v["coverage"] = _coverage_counts(cov.get(v["id"], []))
    return {"start": lo, "end": hi, "count": len(visits), "visits": visits}
...
@router.get("/{visit_id}/coverage")
def coverage(visit_id: int, admin: bool = Depends(is_admin)):
    """Every article reporting this visit — the visit family's own articles
    (keeper + merged rows; no feed-cluster siblings) — with source side and
    sentiment, so the card can lay Taiwan-side and PRC-side coverage next to
    each other.
    Approved visits only; public callers see feed-visible articles only."""
    with db_conn() as conn:
        row = conn.execute(
            "SELECT approval_status FROM cross_strait_visits WHERE id = ?", (visit_id,)).fetchone()
        if not row or (row["approval_status"] != "approved" and not admin):
            raise HTTPException(404, f"visit {visit_id} not found")
        arts = _coverage_rows(conn, [visit_id], admin=admin).get(visit_id, [])
    return {"visit_id": visit_id, "counts": _coverage_counts(arts), "articles": arts}
