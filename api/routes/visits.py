...
# folds the other articles on the same trip into it as `merged` rows. Each
# of those articles may also sit in a feed event cluster (articles.
# event_cluster_id, regenerated by cluster_events.py every tick) with the
# other outlets' coverage of the same story. Coverage = the visit family's
# own articles ('via visit') plus every visible article sharing a cluster
# with one of them ('via cluster'), so both sides' reporting of one trip sits
# together on the card. Sides come from shared/source_side.py (the LinkedIn
# selector's rule); a cluster sibling from an international outlet counts
# towards the article total but towards neither side.
...
cl AS (
    SELECT DISTINCT own.visit_id, a.event_cluster_id AS cluster_id FROM own
    JOIN articles a ON a.id = own.article_id
    WHERE a.event_cluster_id IS NOT NULL
),
reach AS (
    SELECT visit_id, article_id, 'visit' AS via FROM own
    UNION ALL
    SELECT cl.visit_id, a.id, 'cluster' FROM cl
    JOIN articles a ON a.event_cluster_id = cl.cluster_id
)
SELECT r.visit_id, r.via,
       a.id, a.url, a.published_at, CAST(a.event_cluster_id AS TEXT) AS cluster_id,
...
def _coverage_rows(conn, visit_ids, admin: bool = False) -> dict:
    ...
    'via visit' winning over 'via cluster' when both reach it, oldest
    first. Public callers see only feed-visible articles."""
    ...
    for r in conn.execute(sql, ids):
        ...
        vid = d.pop("visit_id")
        seen = out[vid]
        if d["id"] in seen and seen[d["id"]]["via"] == "visit":
            continue
...
def coverage(visit_id: int, admin: bool = Depends(is_admin)):
    ...
    plus their feed-cluster siblings — with source side and sentiment, so
    the card can lay Taiwan-side and PRC-side coverage next to each other.
