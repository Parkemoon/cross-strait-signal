...
### 2026-09-03 — Ops: prod backlog cleared; manifest tags fixed

`bulk_approve_articles.py --renormalise --apply` on prod: 253 articles approved (19,745 → 19,998), 10 held for the Review tab (four Tier-1/2 disagreements, six reported-speech-axis flags), no entity drift. Two analyst scripts named their revert manifests "staging" when run from the prod worktree without `--db` — `bulk_approve_articles.py` and `dedup_visits.py` now tag by the resolved DB path and record it in the manifest header (the untracked bulk-approve script was synced to the prod worktree by hand, per its `.gitignore` note).

### 2026-09-03 — Feed attribution: side first, outlet in full, side tint (deployed to prod same day)

Ed: "it's quite hard to tell which outlet is behind each article." The card's metadata line now opens with the source's side, then the alignment marker, then the publication named in full — `TAIWAN · ■ Liberty Times`, `PRC · ■ China News Service` — with the abbreviation kept under 768px and in the Voices strip. Each card's ground carries a faint wash of its outlet's alignment colour (6%), so a column reads red / green / blue at a glance; the left rule keeps its urgency meaning and an active alt-model lens tint still wins. The tint is per alignment rather than per outlet (thirty tints would be unlearnable) or per bare side (Taiwan has no single colour). `PUBLICATION_NAMES` moved from StatsSidebar into `SourceBadge.jsx` beside `SOURCE_ABBREV`, with new `PLACE_LABEL` and `alignmentTint()`.
...
### 2026-09-02 — Redesign phase 2, step A: document headers + hairline stat bands (deployed to prod same day)
...
### 2026-09-04 — LinkedIn post proposer (DEPLOYED to prod same day, `0d47659`)

Selects ONE cross-strait story and emails a drafted LinkedIn post; never posts anywhere, never touches the editorial pipeline, review queue or public site. **Selector** `shared/linkedin_selector.py` (deterministic, pure ranking, unit-tested): event clusters with a member in the last 48 h and at least one member past the approval gate (the feed's own `_PUBLIC_VISIBLE` predicate) — pending members are allowed but invisible (Ed's call after the dry run: only approved articles feed sides, means, topic and the draft material, so nothing unvetted is quoted); dominant topic not in `EXCLUDED_TOPICS` (`POL_DOMESTIC_TW` + `NOT_RELEVANT`, confirmed by Ed); at least one Taiwan-side outlet (place TW, bias green…blue — place is required because Zaobao/BBC are also `centrist`) AND one PRC-side outlet (state_official / state_nationalist / china_centrist, so RTHK + Ming Pao count); ranked by cross-side breadth = harmonic mean of distinct outlets per side (3+3 → 3.0 beats 6+1 → 1.71; section feeds collapse to the publication, UDN Breaking = UDN), then |TW mean sentiment − PRC mean|, then article count; never re-proposes a cluster id OR any cluster sharing an article with one already proposed (cluster ids are regenerated every tick by `cluster_events.py`, so the id alone is not a stable story identity). **Draft generator** `scraper/processors/linkedin_draft.py`: gemini-3.5-flash writes only the four prose pieces (headline / what happened / TW framing / PRC framing) from the STORED titles, summaries and key quotes; labels, the code-computed mean scores and the feed link are assembled deterministically; `validate_post` enforces <1300 chars, first line ≤210, no em/en-dashes, no hashtags, and that every Chinese run is a substring of a stored key quote — one retry with the violations fed back, then the email is marked NEEDS EDIT rather than shipped silently. Reuses the Tier-1 rule blocks (analyst intro, romanisation, British English, dynamic glossary) — the weekly digest itself uses no Gemini, so those were the nearest existing rule blocks. Gotcha: gemini-3.5-flash spends ~2k thinking tokens out of `max_output_tokens`, so a 2000 budget truncated every response (`Unterminated string`); now 8000 + `thinking_level=low`. **Dry run** `--dry-run-days 14 --db <prod> --out …` (prod DB opened read-only) → `LINKEDIN_DRY_RUN_2026-09-04.md` (untracked): 13 of 14 simulated 07:00-London runs found a candidate, every draft passed validation first attempt; under the lenient rule the 3 Sep run picks the PRC marine survey east of Taiwan (breadth 4.44, 16 approved + 2 pending). **Log table** migration 0011 `linkedin_drafts` (cluster_id TEXT, article_ids JSON, draft, ranking_factors JSON, created_at, emailed_at, emailed_to; mirrored in `schema.sql`; applied on staging) — dedup + audit only, row written only after a successful send. **Email**: `send_email` lifted out of `weekly_digest.py` into `scraper/utils/mail.py` (digest unchanged in behaviour; the health and alt-model scripts still carry their own copies); plain-text body = copy-ready draft, source links per side, two runner-ups one line each with links, ranking factors, exclusion counts; recipient `LINKEDIN_TO` → `DIGEST_TO`; nothing sent when nothing qualifies. End-to-end verified on a scratch copy of the prod DB: real email delivered, row recorded, second run excluded the story as already proposed. **Cron** installed on the server (Tue/Thu; `0 6,7 * * 2,4` with a `TZ=Europe/London` hour test because the box is UTC and vixie cron has no `CRON_TZ`; `-f` guard keeps it silent until deploy) → `/var/log/cross-strait-linkedin.log`. Also lifted the ±0.3 sentiment band into `shared/sentiment_band.py` (digest + proposer; Python twin of `sentimentBand.js`). Surfaced: `articles.event_cluster_id` is declared INTEGER but receives 8-hex-char ids, so type affinity coerces numeric-looking ones (`'1e999999'` → REAL `Inf`) — every reader must compare via `CAST(... AS TEXT)`; the schema comment on `ai_analysis.sentiment_score` still describes the pre-v3 polarity. Tests: `tests/test_linkedin_selector.py` (14); full suite 196. Docs: CLAUDE.md script bullet, database.md, deployment.md cron list.

### 2026-09-04 — Redesign phase 2B: admin surfaces, modals, rail entity markers, roles, Trade Access band (DEPLOYED to prod same day, `e798b9b`)

The last of the Morning Brief phase-2 list. **Admin chrome** — new `components/adminChrome.jsx`: one `ModalFrame` (paper panel, hair border, 2px accent top rule, Newsreader title, scrolling body, hairline footer) and one `Btn` with four verbs (`primary` solid ink · `outline` · `ghost` · `danger`), plus the field/label style tokens. All thirteen analyst modals and queues moved onto it (exercise / diplomacy / poll-colour / poll-entry / roster / positions / visits / copy edits, the four review queues, key-figure curation) — the green/blue/red solid buttons are gone from every admin surface, `pollFormShared`'s `fieldStyle`/`labelStyle` now return the shared tokens so every edit grid restyled at once. **Admin › Review** rebuilt to the design's §10 card (⚑ reason in `--flag`, MODEL READ score in Newsreader 24 beside the DESK selects that flip to "overriding", one primary action: `Accept −0.60` or `Save override` + `Keep model`, `Exclude` as the ghost) with the behaviour untouched. **Admin › Alt Models** is the §11 table (Archivo header on an ink rule, Newsreader 18 figures) over the live groups + definitions / findings / refusal browser under section rules; **AltModelLens** is a hairline row with an Archivo segmented control. `DocumentHeader` gained `eyebrowColour` (amber eyebrow on both admin pages). **Rail entity markers** — `src/entityAlignment.js`: the KEY ENTITIES squares now carry the filled/hollow alignment rule (roster side/party → curated organisation substring table → curated people table; unknown/third-country stays dot-grey). **Key-figure roles** — `role_zh` added to `key_figures.json` for all ten (總統, 國台辦發言人 …), served by `/api/stats/key-figures`, and the Voices attribution reads `NAME 中文名 role_zh` over `PARTY · ROLE` per the design's no-bare-names rule. **Trade Access** — 3-up `StatGrid` band (HS-8 lines banned · ECFA lines suspended · food exporters suspended, one instrument per cell, no cross-side sums), the headline strip became the paired PRC/TW framing blocks, status is typographic, the register table has the ink-ruled Archivo header. Verified on 8082 by Playwright (light + dark; prod review rows and prod 30-day stats routed in because staging's DB has no recent articles) — zero console errors. Screenshots on the artifact page for Ed's review; Ed's go → committed `e798b9b`, FF main, `server_deploy.sh` clean (no migrations, both bundles built, service active), prod API serving `role_zh` confirmed. Phase 2 of the redesign is complete.
