...
### 2026-09-04 — LinkedIn post proposer (staging, uncommitted; cron installed, silent until deploy)
...
- **LinkedIn post proposer**: built on staging (uncommitted) — commit → FF main → `server_deploy.sh` (runs migration 0011); the cron line is already installed and self-activates once the script exists in prod. **After the deploy, drop the `[ -f scripts/propose_linkedin_post.py ] &&` guard from the crontab line** — it exists only for the pre-deploy window, and left in place it would hide a missing script forever. No health-check entry on purpose (a quiet week legitimately sends nothing).
