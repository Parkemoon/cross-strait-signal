-- 0001_baseline: everything the server_deploy.sh inline heredoc used to
-- apply on every deploy (CODE_REVIEW_2026-07-03 §4.2). All statements are
-- idempotent (IF NOT EXISTS / INSERT OR IGNORE / same-value UPDATEs), so
-- applying this to a live DB that already has the objects is a no-op —
-- which is exactly how existing prod/staging DBs get baselined into the
-- schema_migrations regime. New schema work goes in NEW numbered files,
-- not here.
