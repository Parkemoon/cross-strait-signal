import os
import sys

sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
from scraper.utils.db import get_connection

# Broad feeds replaced by targeted sections — deactivate these
DEACTIVATE_SOURCES = [
    'CNA Chinese',    # replaced by CNA Politics/Mainland/International/Finance
    'Liberty Times',  # replaced by LTN Politics/World/Business/Defence
    'Guangming Daily', # anyfeeder proxy dead, rarely cross-strait relevant
]

SOURCES = [
    # Taiwan — targeted sections
    {
        'name': 'LTN Politics',
        'name_zh': '自由時報政治',
        'url': 'https://news.ltn.com.tw/rss/politics.xml',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'green',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'rss',
    },
    {
        'name': 'LTN World',
        'name_zh': '自由時報國際',
        'url': 'https://news.ltn.com.tw/rss/world.xml',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'green',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'LTN Business',
        'name_zh': '自由時報財經',
        'url': 'https://news.ltn.com.tw/rss/business.xml',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'green',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'LTN Defence',
        'name_zh': '自由軍武頻道',
        'url': 'https://def.ltn.com.tw/breakingnewslist',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'green',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'CNA Politics',
        'name_zh': '中央社政治',
        'url': 'https://feeds.feedburner.com/rsscna/politics',
        'source_type': 'state_media',
        'place': 'TW',
        'bias': 'green_leaning',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'rss',
    },
    {
        'name': 'CNA Mainland',
        'name_zh': '中央社兩岸',
        'url': 'https://feeds.feedburner.com/rsscna/mainland',
        'source_type': 'state_media',
        'place': 'TW',
        'bias': 'green_leaning',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'rss',
    },
    {
        'name': 'CNA International',
        'name_zh': '中央社國際',
        'url': 'https://feeds.feedburner.com/rsscna/intworld',
        'source_type': 'state_media',
        'place': 'TW',
        'bias': 'green_leaning',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'CNA Finance',
        'name_zh': '中央社財經',
        'url': 'https://feeds.feedburner.com/rsscna/finance',
        'source_type': 'state_media',
        'place': 'TW',
        'bias': 'green_leaning',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    # Taiwan — blue (China Times targeted sections). chinatimes_scraper.py,
    # headful Chromium: the site challenges headless clients (RSSHub route dead 2026-09-08).
    {
        'name': 'CT Cross-Strait',
        'name_zh': '中時兩岸',
        'url': 'https://www.chinatimes.com/chinese/',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'CT Politics',
        'name_zh': '中時政治',
        'url': 'https://www.chinatimes.com/politic/',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'CT Military',
        'name_zh': '中時軍事',
        'url': 'https://www.chinatimes.com/armament/',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'CT Opinion',
        'name_zh': '中時言論',
        'url': 'https://www.chinatimes.com/opinion/',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'UDN',
        'name_zh': '聯合報兩岸',
        'url': 'https://udn.com/news/cate/2/6640',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'UDN Breaking',
        'name_zh': '聯合報要聞',
        'url': 'https://udn.com/news/cate/2/6638',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'UDN International',
        'name_zh': '聯合報全球',
        'url': 'https://udn.com/news/cate/2/7225',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'UDN Business',
        'name_zh': '聯合報産経',
        'url': 'https://udn.com/news/cate/2/6644',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'YDN',
        'name_zh': '青年日報',
        'url': 'https://www.ydn.com.tw/tw/home/',
        'source_type': 'state_media',
        'place': 'TW',
        'bias': 'green_leaning',
        'language': 'zh-tw',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
        # MND's organ carries almost all ROC domestic drill coverage; its
        # keyword-filter rejects still feed the Step-3b exercise-only pass.
        # Set this on PLA Daily / INDOPACOM-type sources if gaps emerge.
        'exercise_only_scan': 1,
    },
    # TW pollsters — Phase 2d direct ingestion. Step 3c catches all three
    # via the existing %民調% title trigger; no bypass flag needed.
    {
        'name': 'ETtoday Polls',
        'name_zh': 'ET民調',
        'url': 'https://www.ettoday.net/news_search/doSearch.php?keywords=ET%E6%B0%91%E8%AA%BF',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue_leaning',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 720,
        'scrape_method': 'html_scrape',
        'is_pollster_direct': 1,  # article URL IS the pollster's own release page
    },
    {
        'name': 'TVBS Poll Center',
        'name_zh': 'TVBS民調中心',
        'url': 'https://www.tvbs.com.tw/poll-center',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'blue',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 1440,
        'scrape_method': 'html_scrape',
        'is_pollster_direct': 1,  # article URL IS the pollster's own release page
    },
    {
        'name': 'My-Formosa',
        'name_zh': '美麗島電子報',
        'url': 'https://my-formosa.com.tw/Topical/formosapollster',
        'source_type': 'independent_media',
        'place': 'TW',
        'bias': 'centrist',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 1440,
        'scrape_method': 'html_scrape',
        'is_pollster_direct': 1,  # article URL IS the pollster's own release page
    },
    # PRC
    {
        'name': 'Xinhua Chinese',
        'name_zh': '新华社',
        'url': 'https://plink.anyfeeder.com/newscn/whxw',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_official',
        'language': 'zh-cn',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'rss',
    },
    {
        'name': "People's Daily Politics",
        'name_zh': '人民日报台湾',
        'url': 'http://localhost:1200/people/tw',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_official',
        'language': 'zh-cn',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'China News Service',
        'name_zh': '中国新闻网',
        'url': 'https://www.chinanews.com.cn/rss/scroll-news.xml',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_official',
        'language': 'zh-cn',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'Global Times',
        'name_zh': '环球时报台海',
        'url': 'http://localhost:1200/huanqiu/news/taiwan',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_nationalist',
        'language': 'zh-cn',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'The Paper',
        'name_zh': '澎湃新聞',
        'url': 'http://localhost:1200/thepaper/featured',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_official',
        'language': 'zh-cn',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'Guancha',
        'name_zh': '观察者网',
        'url': 'https://www.guancha.cn/taihaifengyun',
        'source_type': 'independent_media',
        'place': 'PRC',
        'bias': 'state_nationalist',
        'language': 'zh-cn',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'China Taiwan Net',
        'name_zh': '中国台湾网',
        'url': 'http://www.taiwan.cn/',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_official',
        'language': 'zh-cn',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'Haixia Daobao',
        'name_zh': '海峽導報',
        'url': 'http://taihai.fjsen.com/',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_official',
        'language': 'zh-cn',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'html_scrape',
    },
    {
        'name': 'PLA Daily',
        'name_zh': '解放軍報',
        'url': 'http://www.81.cn/fyr/',
        'source_type': 'state_media',
        'place': 'PRC',
        'bias': 'state_official',
        'language': 'zh-cn',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'html_scrape',
    },
    # Hong Kong
    {
        'name': 'Ming Pao Cross-Strait',
        'name_zh': '明報兩岸',
        'url': 'https://news.mingpao.com/rss/ins/s00004.xml',
        'source_type': 'independent_media',
        'place': 'HK',
        'bias': 'china_centrist',
        'language': 'zh-hk',
        'tier': 1,
        'scrape_interval': 240,
        'scrape_method': 'rss',
    },
    {
        'name': 'Ming Pao Editorial',
        'name_zh': '明報社評',
        'url': 'https://news.mingpao.com/rss/pns/s00003.xml',
        'source_type': 'independent_media',
        'place': 'HK',
        'bias': 'china_centrist',
        'language': 'zh-hk',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'Ming Pao Opinion',
        'name_zh': '明報觀點',
        'url': 'https://news.mingpao.com/rss/pns/s00012.xml',
        'source_type': 'independent_media',
        'place': 'HK',
        'bias': 'china_centrist',
        'language': 'zh-hk',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'RTHK Greater China',
        'name_zh': '香港電台大灣區',
        'url': 'https://rthk9.rthk.hk/rthk/news/rss/c_expressnews_greaterchina.xml',
        'source_type': 'state_media',
        'place': 'HK',
        'bias': 'state_official',
        'language': 'zh-hk',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    # International Chinese-language Sources
    {
        'name': 'BBC Chinese',
        'name_zh': 'BBC中文',
        'url': 'https://feeds.bbci.co.uk/zhongwen/trad/rss.xml',
        'source_type': 'independent_media',
        'place': 'UK',
        'bias': 'centrist',
        'language': 'zh-tw',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
    {
        'name': 'Zaobao Cross-Strait',
        'name_zh': '聯合早報中港台',
        'url': 'http://localhost:1200/zaobao/realtime/china',
        'source_type': 'independent_media',
        'place': 'SG',
        'bias': 'centrist',
        'language': 'zh-cn',
        'tier': 2,
        'scrape_interval': 360,
        'scrape_method': 'rss',
    },
]


def seed_sources():
    conn = get_connection()
    cursor = conn.cursor()

    # Deactivate broad feeds replaced by targeted sections
    for name in DEACTIVATE_SOURCES:
        cursor.execute("SELECT id FROM sources WHERE name = ?", (name,))
        if cursor.fetchone():
            cursor.execute("UPDATE sources SET is_active = 0 WHERE name = ?", (name,))
            print(f"  Deactivated: {name}")

    for source in SOURCES:
        cursor.execute("SELECT id FROM sources WHERE name = ?", (source['name'],))
        if cursor.fetchone():
            cursor.execute(
                "UPDATE sources SET bias = ?, url = ?, scrape_method = ?, is_active = 1, "
                "is_pollster_direct = ?, exercise_only_scan = ? WHERE name = ?",
                (source['bias'], source['url'], source['scrape_method'],
                 source.get('is_pollster_direct', 0), source.get('exercise_only_scan', 0),
                 source['name']))
            print(f"  Updated: {source['name']}")
            continue

        cursor.execute("""
            INSERT INTO sources
                (name, name_zh, url, source_type, place, bias, language, tier,
                 scrape_interval, scrape_method, is_pollster_direct, exercise_only_scan)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            source['name'], source['name_zh'], source['url'], source['source_type'],
            source['place'], source['bias'], source['language'], source['tier'],
            source['scrape_interval'], source['scrape_method'],
            source.get('is_pollster_direct', 0), source.get('exercise_only_scan', 0)
        ))
        print(f"  Added: {source['name']}")

    conn.commit()
    conn.close()
    print("\nDone. Sources seeded.")


if __name__ == '__main__':
    seed_sources()
