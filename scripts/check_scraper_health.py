...
TABLE_CHECKS = [
    ...
    ("maritime:civil_presence",
     "SELECT MAX(date) FROM maritime_civil_daily", 12,
     "rides on the Step 2n GFW presence pull; same lag as coast_guard:presence"),
    ("maritime:sar",
     "SELECT MAX(date) FROM maritime_sar_daily", 100,
     "Sentinel-1 SAR via GFW (Step 2p): product lands ~2 months late (measured 2026-09-06); 100 = lag + a month"),
